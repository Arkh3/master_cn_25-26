...
def newton_raphson(
    ...
    aux = f(seed)/df_dx(seed)

    i = 0    
    
    while (abs(aux) > tol_abs) and i < max_iters:
        if df_dx(seed) == 0:
            return float('NaN'), float('NaN')
...
def vectorized_newton_raphson(
    ...
    Examples:
        >>> f = lambda x: np.cos(x) - 0.1 * x
        >>> df_dx = lambda x: -np.sin(x) - 0.1
        >>> seeds = np.array([0.1, np.pi, 0.0])
        >>> f_zeros, errors = nc.vectorized_newton_raphson(f, df_dx, seeds, max_iters=20)
        >>> 
        >>> for f_zero, error in zip(f_zeros, errors):
        >>>     format_string = 'Zero of f = {:.4g} ({:.2g}),   '
        >>>     format_string += 'f(f_zero) = {:.2e}.'
        >>>     print(format_string.format(f_zero, np.abs(error), f(f_zero)))
        Zero of f = 5.267 (1.1e-09),   f(f_zero) = -2.22e-16.
        Zero of f = -9.679 (2.8e-09),   f(f_zero) = 3.33e-16.
        Zero of f = 11.72 (1),   f(f_zero) = -5.10e-01.
    """

    if df_dx is not None:
        # Newton-Raphson method
        zeroes_index = np.where(df_dx(seeds) == 0.0)

        if zeroes_index:
            seeds[zeroes_index] = float('NaN')

        aux = f(seeds)/df_dx(seeds)
    
        i = 0    

        while np.any((np.abs(aux) > tol_abs) | (np.isnan(aux))) and i < max_iters:
            zeroes_index = np.where(df_dx(seeds) == 0.0)
        ...
        # Secant method
        # We will use the first and second elements of seeds as the two values
        # to start the secant method from
        x_low = seeds[0]
        x_up = seeds[1]
        ...
        while (abs(x_up - x_low) > tol_abs) and i < max_iters:
            
            # Avoid dividing by 0
            if f(x_up)-f(x_low) == 0:
                return float('NaN'), float('NaN')
            
            x_m = x_up - (((x_up-x_low) / (f(x_up)-f(x_low)) * f(x_up)))
            error = None
...
def bisection(
    ...
    while (error > tol_abs) and i < max_iters:
